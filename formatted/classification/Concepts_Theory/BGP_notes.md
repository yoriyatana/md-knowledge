...
Specify the number of times detection of the AS number in the AS_PATH attribute causes the route to be discarded or hidden.
...
- Đối với **IBGP**: sẽ có 3 trường hợp, cụ thể:
    - Các routes xuất phát _bên trong AS_ đã có sẵn **forwarding next-hop** thì next-hop của các route đó khi quảng bá qua BGP sẽ lấy forwarding next-hop này **(third-party next-hop)**
    - Các routes xuất phát _bên trong AS_ với next-hop là **reject** hoặc **discard**, khi đó next-hop được quảng bá vào BGP sẽ là **địa chỉ IP** đang dùng để **kết nối** phiên BGP.
...
![c77a0edfbc5f2d627d04451ab74380ee.png](image/c77a0edfbc5f2d627d04451ab74380ee.png)
...
![4c54e4782944ef074635ecfaecaa98b8.png](image/4c54e4782944ef074635ecfaecaa98b8.png)
...
![96219a6e02e0b2cd95c9e14df2f05e55.png](image/96219a6e02e0b2cd95c9e14df2f05e55.png)

![b1eaa9c5ad90409336e54e94005e2a82.png](image/b1eaa9c5ad90409336e54e94005e2a82.png)

To summarize:

- **Active** state - local router has just _sent_ a TCP SYN
- **Connect** state - local router has just _received_ a TCP SYN from it's peer
...
Notice, only the peer which Initiated the TCP handshake passes through Active state. And only the peer which _did NOT_ initiate the TCP handshake passes through the Connect state.
...
![d405eb1219dcb94b395cb8c0840c74d3.png](image/d405eb1219dcb94b395cb8c0840c74d3.png)

...
[![](image/e64dd97d311f08e1158056e6290402ca.png)](http://inetzero.com/wp-content/uploads/2014/06/ldp-1.png)
...
[![](image/0018d7d9cd094b3c7004ba211e706c22.png)](http://inetzero.com/wp-content/uploads/2014/06/ldp-3.png)Note: To override those default parameters you can use those following commands:

[![](image/b3b8fa0848870df564768e4b6169d465.png)](http://inetzero.com/wp-content/uploads/2014/06/ldp-9.png)

[2] PE1 and P1 establish between them a TCP session and start to exchange the LDP session parameters:

[![](image/46f6d30146ca06af992ff423aff94ec5.png)](http://inetzero.com/wp-content/uploads/2014/06/ldp-4.png)
...
[![](image/0b90d9d776a9b292c2f28666b78574e2.png)](http://inetzero.com/wp-content/uploads/2014/06/ldp-10.png)
...
[![](image/19ed1ddf82d019257306166f5668417f.png)](http://inetzero.com/wp-content/uploads/2014/06/ldp-142.png)Then call back the command to check the inet.3 routing table. Great! As you can see know LDP metrics are mapped to IGP metrics.

[![](image/e506f183715f8ac897180d17063a505c.png)](http://inetzero.com/wp-content/uploads/2014/06/ldp-151.png)
...
[![](image/dcbc9ba59285d80ed7a7f4e5561dc289.png)](http://inetzero.com/wp-content/uploads/2014/06/ldp-183.png)Commit this and then check LDP Database of PE1:

[![](image/340a0e6070e4f294f2d38fac6bf215a2.png)](http://inetzero.com/wp-content/uploads/2014/06/ldp-23.png)Great! Now PE1 maps a label to its Secondary Lo0 address.
...
[![](image/399618e58888dcd04231156c6b2afe93.png)](http://inetzero.com/wp-content/uploads/2014/06/ldp-24.png)The term2 reject all other routes that do not match the range 192.167.2.0/24 (remember the default import policy is accept all). No let’s commit this configuration on PE1 and check again the LDP database of this router:

[![](image/3d5a542d078afd3293e1bd700d3f39c4.png)](http://inetzero.com/wp-content/uploads/2014/06/ldp-25.png)The result is quite different with an Export policy. Indeed, as only PE1 advertises its Secondary address, the Label Mapping received from P1 concerns only Primary Addresses, except the PE1 label mapping sent back by P1. So, all those routes are filtered. This is also why PE1 sends only its Egress FEC. It can’t allocate a Label Mapping for a FEC that it didn’t receive and accept a Label mapping from a downstream router before. In this case the PE1 inet.3 routing table will be empty.
...
[![](image/50d68996b9165030d19aec1dd370eb88.png)](http://inetzero.com/wp-content/uploads/2014/06/ldp-26.png)By default, Junos aggregates in a single FEC. Let’s check on P1 its LDP database:

[![](image/2f36f4e1aaa4b4f96998dc60d7c9176e.png)](http://inetzero.com/wp-content/uploads/2014/06/ldp-27.png)As you can see, P1 receives from PE1 2 EGRESS IP addresses (map to implicit null label 3 because PE1 is egress PE for those 2 addresses), but the most interesting part is the label allocated back by P1 for those 2 addresses. As you can see, P1 “aggregates” the 2 IPs addresses in a single FEC and allocates the same label (299856).

Now let’s try to configure the deaggregate knob on P1 like that:

[![](image/235cfd21dab7eecdc370c68db21eec7c.png)](http://inetzero.com/wp-content/uploads/2014/06/ldp-28.png)and have a look gain on P1 LDP database:

[![](image/386a9961a3c49427942f52d41d2ff263.png)](http://inetzero.com/wp-content/uploads/2014/06/ldp-29.png)
...
[![](image/491ffb9211b788648e25aec4442a780d.png)](http://inetzero.com/wp-content/uploads/2014/06/ldp-30.png)Now, you can check on PE1 the LDP Database and the inet.3 table:

[![](image/6fc05c2d0cf240ed6e6863584a470ee5.png)](http://inetzero.com/wp-content/uploads/2014/06/ldp31-.png)Now P1 allocates a label of 0 for its primary loopback address. To reach the P1 Next-hop, PE1 will explicitly push the label 0.
...
[![](image/6a4fcc6e9440506e86d0736b592f0ab8.png)](http://inetzero.com/wp-content/uploads/2014/06/ldp-32.png)Now we disable LDP on P1 but we keep OSPF UP. On P2 the LDP session comes down:

[![](image/0bf5673e621d9a52297e8f539fb76c64.png)](http://inetzero.com/wp-content/uploads/2014/06/ldp-33.png)Let’s check the P2 OSPF LSAs – you can see that the adjacency of P1 is now assigned to an infinite metric. The “show route ” confirms also the right behaviour:

[![](image/ce3f43ffe5174a0af2d77e8988fe7fdd.png)](http://inetzero.com/wp-content/uploads/2014/06/ldp-34.png)
...
[![](image/1c120fc52c6b7e97f6e39c52805c6478.png)](http://inetzero.com/wp-content/uploads/2014/06/ldp-35.png)Note: you can specify the number of seconds before the LDP session is torn down and resignaled. To do that, use the parameter “timeout” behind the session-protection knob.

Now we can check back the LDP Discovery adjacencies on PE2:

[![](image/e69530714442021d241271af6fd08b8f.png)](http://inetzero.com/wp-content/uploads/2014/06/ldp-36.png)
...
[![](image/2cf4ec57e40546c4a512c838aab44a10.png)](http://inetzero.com/wp-content/uploads/2014/06/ldp-37.png)Now, check back, LDP Discovery adj. LDP session and LDP database on PE2:

[![](image/b3c72649203c6ed8e43fe3999b6a4578.png)](http://inetzero.com/wp-content/uploads/2014/06/ldp-38.png)Great, all works as expected. Only the classical/Direct Discovery adjacency between PE2 and P2 has disappeared. The Lo0 Discovery Adj stays up, because targeted hellos are sent through the “backup” path PE2P3P2 (possible because TTL is upper than 1 in this case). The LDP TCP session between PE2 and P2 is still up and the database has not been flushed. when link between PE2 / P2 will come back up only the direct discovery adj. will be re-established and that all, PE2 and P2 will be already synchronized.

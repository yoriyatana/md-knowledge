...
1. > lab> show chassis routing-engine
    >
    ...
    Check the CPU utilization on the Junos device with the command:    show chassis routing-engine
    ...
    - For more information about the output of the 'show chassis routing-engine' command, refer to the following link:
        [http://www.juniper.net/techpubs/en_US/junos/topics/reference/command-summary/show-chassis-routing-engine.html](http://www.juniper.net/techpubs/en_US/junos/topics/reference/command-summary/show-chassis-routing-engine.html)
        * *Idle** : Percentage of CPU time that is idle.
2. If the **User** or **Background** process is high, continue to step 2 for further analysis.
3. If the **Interrupt** process is high, jump to [Interrupt process consuming High CPU](https://supportportal.juniper.net/s/article/M-MX-T-series-Troubleshooting-Checklist-Routing-Engine-High-CPU?language=en_US#interrupt) .
...
4. If the **NTPD** process is high, jump to [NTPD_process_consuming_High_CPU](https://supportportal.juniper.net/s/article/M-MX-T-series-Troubleshooting-Checklist-Routing-Engine-High-CPU?language=en_US#NTPD_process_consuming_High_CPU) .
    - --
    If the **Cscript** process is high, jump to [CSCRIPT_process_consuming_High_CPU](https://supportportal.juniper.net/s/article/M-MX-T-series-Troubleshooting-Checklist-Routing-Engine-High-CPU?language=en_US#CSCRIPT_process_consuming_High_CPU) .
5. lab > show system processes extensive
    ...
    * *2307 root 1 4 0 42156K 19240K kqread 8:11 93.05% rpd <---------------**
    ...
    [http://www.juniper.net/techpubs/en_US/junos/topics/reference/command-summary/show-system-processes.html](http://www.juniper.net/techpubs/en_US/junos/topics/reference/command-summary/show-system-processes.html)
    Check the CPU utilization consumed by each software process with the command:    show system process extensive
6. Check the output of the following commands around the time of the High CPU utilization event for any additional clues:
    - For more information on the above command output please refer to the following:
        [https://www.juniper.net/techpubs/en_US/junos/topics/reference/command-summary/show-system-virtual-memory.html](https://www.juniper.net/techpubs/en_US/junos/topics/reference/command-summary/show-system-virtual-memory.html)
        show system virtual-memory | no-more
    - For more information on the above command output please refer to the following:
        [http://www.juniper.net/techpubs/en_US/junos/topics/reference/command-summary/show-task-memory.html](http://www.juniper.net/techpubs/en_US/junos/topics/reference/command-summary/show-task-memory.html)
        show task memory detail | no-more
    - show log messages | no-more
7. [KB22637 - Data Collection Checklist - Logs/data to collect for troubleshooting](https://supportportal.juniper.net/s/article/M-MX-PTX-T-Data-Collection-Checklist-Logs-data-to-collect-for-troubleshooting) .
    If you need to open a case with your technical support representative, collect the output specified in the 'High CPU' section of the Data Collection Checklist:
...
5. user@router> show task accounting
...
6. [http://www.juniper.net/techpubs/en_US/junos/topics/reference/general/rpd-memory-faq.html](http://www.juniper.net/techpubs/en_US/junos/topics/reference/general/rpd-memory-faq.html) .
    Check RPD Memory. Some times High memory utilization might indirectly lead to high CPU, to interpret RPD memory utilization in Junos, refer to
7. [KB22637 - Data Collection Checklist - Logs/data to collect for troubleshooting](https://supportportal.juniper.net/s/article/M-MX-PTX-T-Data-Collection-Checklist-Logs-data-to-collect-for-troubleshooting) and open a case with your technical support representative.
    If still the rpd is consuming high cpu then collect the logs from data collection checklist
...
- Apr 23 17:12:37.666 2010 igw02.brf-re0 /kernel: %KERN-3-KERN_ARP_DUPLICATE_ADDR:
    duplicate IP address 10.55.1.219! sent from address: 00:a0:a5:64:2d:6c (error count = 3672452574)
    Apr 23 17:13:37.632 2010 igw02.brf-re0 /kernel: %KERN-3-KERN_ARP_DUPLICATE_ADDR:
    duplicate IP address 10.55.1.219! sent from address: 00:a0:a5:64:2d:6c (error count = 3675570687)
    Apr 23 17:14:37.598 2010 igw02.brf-re0 /kernel: %KERN-3-KERN_ARP_DUPLICATE_ADDR:
...
One of the symptoms that occur with high kernel CPU usage are messages with RPD_SCHED_SLIP in the logs.
...
[http://www.juniper.net/techpubs/en_US/junos14.1/topics/task/configuration/script-dampening-configuration.html](http://www.juniper.net/techpubs/en_US/junos14.1/topics/task/configuration/script-dampening-configuration.html)

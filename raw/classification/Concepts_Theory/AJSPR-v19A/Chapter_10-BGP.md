...
Required to accept routes with your own AS in the AS-path. Specify the number of times detection of the AS number in the AS_PATH attribute causes the route to be discarded or hidden.
...
- Other options are the following:
    ...
    - **• local-as loops** **_number:_** Specify the maximum number of times that the local AS number can appear in an AS path received from a BGP peer. For number, include a value from1 through 10.

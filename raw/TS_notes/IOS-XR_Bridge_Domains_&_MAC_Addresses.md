# IOS-XR Bridge Domains & MAC Addresses
...
|show mac-address-table bdomain <bridge-domain-id>  |đảm bảo có MAC trên port downlink và qua PW
kiểm tra học MAC trong bridge-domain|           |
|**Clear MAC trong bridge-domain**                  |                                                                               |           |
| clear mac-address-table bdomain <bridge-domain-id>|xóa MAC trong bridge-domain để kiểm tra MAC có học lại không                   |           |
...
|show l2vpn forwarding bridge-domain <bridge-group>:<brigde-domain-name>  mac-address location 0/0/CPU0      |đảm bảo có MAC trên port downlink và qua PW
kiểm tra học MAC trong bridge-domain|           |
|**Clear MAC trong bridge-domain**                                                                           |                                                                               |           |
|clear l2vpn forwarding mac-address-table bridge-domain <bridge-group>:<brigde-domain-name> location 0/0/CPU0|xóa MAC trong bridge-domain để kiểm tra MAC có học lại không                   |           |
...
|show bridge-domain <bridge-domain-id>|phải đảm bảo chứa service-instance, có PW nối đến các MA khác
kiểm tra các member thuộc brigde-domain|           |
...
|show l2vpn vfi name <vfi-name>|yêu cầu vfi phải UP

yêu cầu các PW phải báo hiệu được local label, remote label

kiểm tra giá trị ve-id
kiểm tra PW trong vfi|           |
...
|show l2vpn bridge-domain bd-name <bridge-domain-name>|hiển thị trạng thái PW trong bridge-domain|           |
...
|------------------------------|----------------------------------------------------------------------------------------------------------------------------------------|-----------|
|**ASR920**
**ASR903,**         |**Kiểm tra cấu hình service-instance, bridge-domain**                                                                                   |           |
|show run |  section <vlan>    |hiện thị được khai báo service-instance, bridge-domain vì quy hoạch bằng giá trị vlan
hiển thị tất cả cấu hình liên quan đến vlan dịch vụ|           |
|**Kiểm tra cấu hình VFI**     |                                                                                                                                        |           |
|show run |  section <vfi-name>|hiển thị cấu hình liên quan đến vfi                                                                                                     |           |
...
|--------------------------------|-----------------------------------------------------------------------------------------------------------------------------------|-----------|
|**ASR920**
**ASR903,**           |**Khai báo IP trên interface BDI,** **lưu ý: chỉ ping được từ BDI đến IP khách hàng khi có khai rewrite trong service-instance MA**|           |
|interface <bdi-id>              |giá trị BDI-ID trùng vói bridge-domain
khai báo IP trên interface BDI                                                               |           |
|ip address <dia-chi-ip> <subnet>|khai báo địa chỉ IP thuộc subnet LAN khách hàng                                                                                    |           |
